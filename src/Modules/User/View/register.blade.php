...
{{--    {{dump($errors)}}--}}
    <x-form.auth-form
        ...
        <x-form.primary-button>Зарегистрироваться</x-form.primary-button>

        <x-slot:social-auth>
            <a href="#"
               class="mt-5 relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs"><a href="{{route('user.login')}}" class="text-white hover:text-white/70 font-bold">Войти</a>
                </div>
            </div>
        </x-slot:buttons>
    </x-form.auth-form>

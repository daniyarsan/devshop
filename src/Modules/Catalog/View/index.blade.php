...
@section('content')
    ...
    <section>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5 mt-8">
            @each('View._parts.category', $categories, 'item', '')
        </div>
    </section>
    ...
    <section class="mt-16 lg:mt-24">
        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
            ...
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{route('catalog.index', $category)}}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
                    @foreach(filters() as $filter)
                        @include($filter->view(), ['filter' => $filter])
                    @endforeach

                    <div class="space-y-10">
                        <button type="submit" class="w-full !h-16 btn btn bg-amber-700">Фильтровать</button>
            ...
            <div class="basis-auto xl:basis-3/4">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <form x-ref="sortForm" action="{{route('catalog.index')}}">
                            <select x-on:change="$refs.sortForm.submit()" name="sort" class="form-select w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xxs sm:text-xs shadow-transparent outline-0 transition">
                ...
                <div class="products grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-6 2xl:gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12">
                    @if($products->isNotEmpty())
                        @each('View._parts.product', $products, 'item', '')
                    @endif
                </div>
